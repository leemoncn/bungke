  ...
  <el-container style="height: 100%" class="app-container">
    <el-main>
      <div class="filter-container">
        <el-form :inline="true" :model="searchForm" ref="searchForm">
          ...
          <el-form-item label="支持设备类型" prop="mobilePropertyId">
            <el-input v-model="searchForm.mobilePropertyId" autocomplete="new-password"/>
          </el-form-item>
          <el-form-item label="标题" prop="title">
            <el-input v-model="searchForm.title" autocomplete="new-password"/>
          </el-form-item>
      ...
      <el-table :data="tableData" style="width: 100%" tooltip-effect="dark" border fit @sort-change="onTableSortChange"
        ...
        <el-table-column prop="typePropertyId" label="任务类型" sortable="custom"/>
        <el-table-column prop="userId" label="发布人id" sortable="custom" align="center">
          <template v-slot="scope">
            <user-popover :data="scope.row.userId"/>
          </template>
        </el-table-column>
        <el-table-column prop="mobilePropertyId" label="支持设备类型" sortable="custom"/>
        <el-table-column prop="title" label="标题" sortable="custom"/>
        <el-table-column prop="deadlineTime" label="截止时间" sortable="custom"/>
        <el-table-column prop="price" label="出价" sortable="custom"/>
        <el-table-column prop="count" label="任务数量" sortable="custom"/>
        <el-table-column prop="feePrice" label="服务费金额" sortable="custom"/>
        <el-table-column prop="feePercent" label="手续费百分比" sortable="custom"/>
        <el-table-column prop="missionRuleId" label="任务规则id" sortable="custom"/>
        <el-table-column prop="url" label="链接" sortable="custom"/>
        ...
        <el-table-column label="操作" fixed="right" width="210">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-permission="'core_mission_detail_edit'">编辑
            </el-button>
            <el-button
              size="mini"
              type="danger"
              @click.stop="onItemDeleteButtonPressed(scope.row)"
              v-permission="'core_mission_detail_delete'">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  import { COREMISSIONDETAIL_DELETE_URL, COREMISSIONDETAIL_LIST_URL } from "@/config/host"
  ...
  export default {
    name: "CoreMissionDetailManagement",
    ...
    methods: {
      ...
      proceedListRequest: function (isSearch) {
        this.httpPostWithLoading(COREMISSIONDETAIL_LIST_URL, this.createListRequestData(isSearch ? this.searchForm : null)).then(({ data, success }) => {
          if (success) {
      ...
      onItemDeleteButtonPressed: function (row) {
        this.$confirm(`删除节点<${row.id}>, 是否继续?`, "提示", {
          ...
          this.httpPostWithLoading(COREMISSIONDETAIL_DELETE_URL, { id: row.id }).then(({ success }) => {
            if (success) {
              this.$message({
                message: "菜单删除成功",
                type: "success"
