...
<template>
  <el-container style="height: 100%" class="app-container">
    <el-main>
      <div class="filter-container">
        <el-form :inline="true" :model="searchForm" ref="searchForm">
          <el-form-item label="版本号" prop="versionCode">
            <el-input v-model="searchForm.versionCode" autocomplete="new-password"/>
          </el-form-item>
          <el-form-item label="版本名" prop="versionName">
            <el-input v-model="searchForm.versionName" autocomplete="new-password"/>
          </el-form-item>
      ...
      <div class="filter-container">
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-plus" v-permission="'core_version_add'"
                   @click="onAddButtonPressed">添加
        </el-button>
        <!-- <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-edit" v-permission="'core_version_edit'"
                   @click="onEditButtonPressed">修改
        </el-button> -->
        <!--<el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-delete" v-permission="'core_version_delete'"-->
      ...
      <el-table :data="tableData" style="width: 100%" tooltip-effect="dark" border fit @sort-change="onTableSortChange"
        ...
        <el-table-column prop="versionCode" label="版本号" sortable="custom" align="center"/>
        <el-table-column prop="versionName" label="版本名" sortable="custom" align="center"/>
        <el-table-column prop="forceUpdate" label="强制更新" sortable="custom" align="center"
                         :formatter="(row, column, cellValue, index)=>cellValue?'是':'否'"/>
        <el-table-column prop="deployed" label="是否部署完成" align="center"
                         :formatter="(row, column, cellValue, index)=>cellValue?'是':'否'"/>
        ...
        <el-table-column label="操作" fixed="right" width="210">
          <template slot-scope="scope">
            ...
            <!-- <el-button
              ...
              v-permission="'core_version_edit'">编辑
            </el-button> -->
            <!--<el-button-->
            <!--size="mini"-->
            <!--type="danger"-->
            <!--@click.stop="onItemDeleteButtonPressed(scope.row)"-->
            <!--v-permission="'core_version_delete'">删除-->
            <!--</el-button>-->
          </template>
        </el-table-column>
      </el-table>
      ...
      <edit-dialog :edit-dialog-data="editDialogData" :edit-dialog-visible.sync="editDialogVisible"
                   @reload="loadTableData" :last-version="lastVersion"
...
<script>
  import { COREVERSION_DELETE_URL, COREVERSION_LIST_URL } from "@/config/host"
  ...
  export default {
    ...
    methods: {
      ...
      proceedListRequest: function (isSearch) {
        this.httpPostWithLoading(COREVERSION_LIST_URL, this.createListRequestData(isSearch ? this.searchForm : null)).then(({ data, success }) => {
          if (success) {
      ...
      onItemDeleteButtonPressed: function (row) {
        this.$confirm(`删除节点<${row.id}>, 是否继续?`, "提示", {
          ...
          this.httpPostWithLoading(COREVERSION_DELETE_URL, { id: row.id }).then(({ success }) => {
            if (success) {
              this.$message({
                message: "菜单删除成功",
                type: "success"
