...
<template>
  <el-container style="height: 100%" class="app-container">
    <el-main>
      ...
      <div class="filter-container">
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-plus" v-permission="'sys_advice_add'"
                   @click="onAddButtonPressed">添加
        </el-button>
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-edit" v-permission="'sys_advice_edit'"
                   @click="onEditButtonPressed">修改
        </el-button>
        <!--<el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-delete" v-permission="'sys_advice_delete'"-->
      ...
      <el-table :data="tableData" style="width: 100%" tooltip-effect="dark" border fit @sort-change="onTableSortChange"
        ...
        <el-table-column prop="haveImg" label="图片">
          <template slot-scope="scope">
            <img-popover :img-data="{dataId:scope.row.id,type:3}" v-if="scope.row.haveImg"/>
            <img-popover :img-path-list="[]" v-else/>
          </template>
        </el-table-column>
        <el-table-column prop="message" label="意见反馈内容" sortable="custom"/>
        <el-table-column prop="contactInfo" label="联系方式" sortable="custom"/>
...
<!--            <el-button-->
            ...
            <!--              v-permission="'sys_advice_edit'">编辑-->
            <!--            </el-button>-->
            <!--<el-button-->
            <!--size="mini"-->
            <!--type="danger"-->
            <!--@click.stop="onItemDeleteButtonPressed(scope.row)"-->
            <!--v-permission="'sys_advice_delete'">删除-->
            <!--</el-button>-->
          </template>
        </el-table-column>
      </el-table>
...
<script>
  import { SYSADVICE_DELETE_URL, SYSADVICE_LIST_URL } from "@/config/host"
  ...
  export default {
    name: "SysAdviceManagement",
    components: { UserPopover, ImgPopover, EditDialog },
    data () {
      return {
        tableData: [],
        editDialogData: null,
    ...
    methods: {
      ...
      proceedListRequest: function (isSearch) {
        this.httpPostWithLoading(SYSADVICE_LIST_URL, this.createListRequestData(isSearch ? this.searchForm : null)).then(({ data, success }) => {
          if (success) {
      ...
      onItemDeleteButtonPressed: function (row) {
        this.$confirm(`删除节点<${row.id}>, 是否继续?`, "提示", {
          ...
          this.httpPostWithLoading(SYSADVICE_DELETE_URL, { id: row.id }).then(({ success }) => {
            if (success) {
              this.$message({
                message: "菜单删除成功",
                type: "success"
