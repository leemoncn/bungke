...
<template>
  <el-container style="height: 100%" class="app-container">
    <el-main>
      <div class="filter-container">
        <el-form :inline="true" :model="searchForm" ref="searchForm">
          <el-form-item label="类型" prop="type">
            <!--<el-input v-model="searchForm.type" autocomplete="new-password"/>-->
            <el-select v-model="searchForm.type" placeholder="请选择" value="">
              <el-option label="请选择" value=""/>
              <el-option v-for="(item,index) in allPropertyType"
                         :key="index" :label="item + '---' + (index+1)" :value="index + 1"/>
            </el-select>
          </el-form-item>
      ...
      <div class="filter-container">
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-plus" v-permission="'sys_property_add'"
                   @click="onAddButtonPressed">添加
        </el-button>
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-edit" v-permission="'sys_property_edit'"
                   @click="onEditButtonPressed">修改
        </el-button>
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-delete"
                   v-permission="'sys_property_delete'"
        ...
        <el-table-column label="操作" fixed="right" width="210">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-permission="'sys_property_edit'">编辑
            </el-button>
            <el-button
              size="mini"
              type="danger"
              @click.stop="onItemDeleteButtonPressed(scope.row)"
              v-permission="'sys_property_delete'">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <edit-dialog :edit-dialog-data="editDialogData" :edit-dialog-visible.sync="editDialogVisible"
                   @reload="loadTableData" :all-property-type="allPropertyType"
...
<script>
  import { SYSPROPERTY_DELETE_URL, SYSPROPERTY_LIST_URL } from "@/config/host"
  ...
  export default {
    ...
    methods: {
      ...
      proceedListRequest: function (isSearch) {
        this.httpPostWithLoading(SYSPROPERTY_LIST_URL, this.createListRequestData(isSearch ? this.searchForm : null)).then(({ data, success }) => {
          if (success) {
      ...
      onItemDeleteButtonPressed: function (row) {
        this.$confirm(`删除节点<${row.name}>, 是否继续?`, "提示", {
          ...
          this.httpPostWithLoading(SYSPROPERTY_DELETE_URL, { id: row.id }).then(({ success }) => {
            if (success) {
              this.$message({
                message: "菜单删除成功",
                type: "success"
      ...
      typeFormatter (row, column, cellValue, index) {
        return this.allPropertyType[row.type - 1]
      }
    }
  }
</script>
