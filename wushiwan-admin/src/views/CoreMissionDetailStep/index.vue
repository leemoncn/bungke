  ...
  <el-container style="height: 100%" class="app-container">
    <el-main>
      <div class="filter-container">
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-plus" v-permission="'core_mission_detail_step_add'"
                   @click="onAddButtonPressed">添加
        </el-button>
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-edit" v-permission="'core_mission_detail_step_edit'"
                   @click="onEditButtonPressed">修改
        </el-button>
        <!--<el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-delete" v-permission="'core_mission_detail_step_delete'"-->
      ...
      <el-table-column prop="missionDetailId" label="missionDetailId" sortable="custom" align="center"/>
      <el-table-column prop="haveimg" label="是否包含图片" sortable="custom" align="center"/>
        ...
        <el-table-column label="操作" fixed="right" width="210">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-permission="'core_mission_detail_step_edit'">编辑
            </el-button>
            <!--<el-button-->
              <!--size="mini"-->
              <!--type="danger"-->
              <!--@click.stop="onItemDeleteButtonPressed(scope.row)"-->
              <!--v-permission="'core_mission_detail_step_delete'">删除-->
            <!--</el-button>-->
          </template>
        </el-table-column>
      </el-table>
...
<script>
  import { COREMISSIONDETAILSTEP_DELETE_URL, COREMISSIONDETAILSTEP_LIST_URL } from "@/config/host"
  ...
  export default {
    name: "CoreMissionDetailStepManagement",
    ...
    methods: {
      ...
      proceedListRequest: function (isSearch) {
        this.httpPostWithLoading(COREMISSIONDETAILSTEP_LIST_URL, this.createListRequestData(isSearch ? this.searchForm : null)).then(({ data, success }) => {
          if (success) {
      ...
      onItemDeleteButtonPressed: function (row) {
        this.$confirm(`删除节点<${row.id}>, 是否继续?`, "提示", {
          ...
          this.httpPostWithLoading(COREMISSIONDETAILSTEP_DELETE_URL, { id: row.id }).then(({ success }) => {
            if (success) {
              this.$message({
                message: "菜单删除成功",
                type: "success"
