    ...
    <el-main>
      ...
      <div class="filter-container">
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-plus"
                   v-permission="'core_partner_purchase_add'"
                   @click="onAddButtonPressed">添加
        </el-button>
        <!--<el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-edit"-->
        <!--v-permission="'core_partner_purchase_edit'"-->
        <!--@click="onEditButtonPressed">修改-->
        <!--</el-button>-->
        <!--<el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-delete" v-permission="'core_partner_purchase_delete'"-->
      ...
      <el-table :data="tableData" style="width: 100%" tooltip-effect="dark" border fit @sort-change="onTableSortChange"
        ...
        <el-table-column prop="partnerId" label="合作商" sortable="custom"
                         :formatter="(row)=>{return getPartnerName(row.partnerId)}"/>
        <el-table-column prop="price" label="花费的钱(元)" sortable="custom"
                         :formatter="(row)=>{return row.price/100.0}"/>
        <el-table-column prop="startTime" label="开始时间" sortable="custom"/>
        <el-table-column prop="endTime" label="结束时间" sortable="custom"/>
        <el-table-column prop="timeType" label="时间" sortable="custom"
                         :formatter="timeTypeFormatter"/>
        <el-table-column prop="usable" label="是否有效" sortable="custom"
                         :formatter="(row)=>{return row.usable?'有效':'失效'}"/>
        ...
        <el-table-column label="操作" fixed="right" width="210">
          <template slot-scope="scope">
            ...
            <!--v-permission="'core_partner_purchase_edit'">编辑-->
            <!--</el-button>-->
            <!--<el-button-->
            <!--size="mini"-->
            <!--type="danger"-->
            <!--@click.stop="onItemDeleteButtonPressed(scope.row)"-->
            <!--v-permission="'core_partner_purchase_delete'">删除-->
            <!--</el-button>-->
          </template>
        </el-table-column>
      </el-table>
      ...
      <edit-dialog :edit-dialog-data="editDialogData" :edit-dialog-visible.sync="editDialogVisible"
                   @reload="loadTableData" :partner-info="partnerInfo"
...
<script>
  import { COREPARTNER_LIST_URL, COREPARTNERPURCHASE_DELETE_URL, COREPARTNERPURCHASE_LIST_URL } from "@/config/host"
  ...
  export default {
    name: "CorePartnerPurchaseManagement",
    ...
    methods: {
      async loadPartnerInfo () {
        const result = await this.httpPostWithLoading(COREPARTNER_LIST_URL, {
          pageSize: 9999
        })
        if (result.success) {
          this.partnerInfo = result.data.records
          console.log("this.partnerInfo = ", this.partnerInfo)
        }
      },
      getPartnerName (partnerId) {
        for (const info of this.partnerInfo) {
          if (info.id === partnerId) {
            return info.name
          }
        }
        return "未知"
      ...
      proceedListRequest: function (isSearch) {
        this.httpPostWithLoading(COREPARTNERPURCHASE_LIST_URL, this.createListRequestData(isSearch ? this.searchForm : null)).then(({ data, success }) => {
          if (success) {
      ...
      onItemDeleteButtonPressed: function (row) {
        this.$confirm(`删除节点<${row.id}>, 是否继续?`, "提示", {
          ...
          this.httpPostWithLoading(COREPARTNERPURCHASE_DELETE_URL, { id: row.id }).then(({ success }) => {
            if (success) {
              this.$message({
                message: "菜单删除成功",
                type: "success"
