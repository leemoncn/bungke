    ...
    <el-main>
      <div class="filter-container">
        <el-form :inline="true" :model="searchForm" ref="searchForm">
          ...
          <el-form-item label="当前合作商是否可用" prop="usable">
            <el-input v-model="searchForm.usable" autocomplete="new-password"/>
          </el-form-item>
      ...
      <div class="filter-container">
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-plus"
                   v-permission="'core_deposit_purchase_add'"
                   @click="onAddButtonPressed">添加
        </el-button>
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-edit"
                   v-permission="'core_deposit_purchase_edit'"
                   @click="onEditButtonPressed">修改
        </el-button>
        <!--<el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-delete" v-permission="'core_deposit_purchase_delete'"-->
      ...
      <el-table :data="tableData" style="width: 100%" tooltip-effect="dark" border fit @sort-change="onTableSortChange"
        ...
        <el-table-column prop="userId" label="用户id" sortable="custom">
          <template v-slot="scope">
            <user-popover/>
          </template>
        </el-table-column>
        <el-table-column prop="deposit" label="保证金金额,单位分" sortable="custom"/>
        <el-table-column prop="refundTime" label="申退保证金日期" sortable="custom"/>
        <el-table-column prop="refundFinishTime" label="申退保证金完成" sortable="custom"/>
        <el-table-column prop="usable" label="当前合作商是否可用" sortable="custom"/>
        ...
        <el-table-column label="操作" fixed="right" width="210">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-permission="'core_deposit_purchase_edit'">编辑
            </el-button>
            <!--<el-button-->
            <!--size="mini"-->
            <!--type="danger"-->
            <!--@click.stop="onItemDeleteButtonPressed(scope.row)"-->
            <!--v-permission="'core_deposit_purchase_delete'">删除-->
            <!--</el-button>-->
          </template>
        </el-table-column>
      </el-table>
...
<script>
  import { COREDEPOSITPURCHASE_DELETE_URL, COREDEPOSITPURCHASE_LIST_URL } from "@/config/host"
  ...
  export default {
    name: "CoreDepositPurchaseManagement",
    ...
    methods: {
      ...
      proceedListRequest: function (isSearch) {
        this.httpPostWithLoading(COREDEPOSITPURCHASE_LIST_URL, this.createListRequestData(isSearch ? this.searchForm : null)).then(({ data, success }) => {
          if (success) {
      ...
      onItemDeleteButtonPressed: function (row) {
        this.$confirm(`删除节点<${row.id}>, 是否继续?`, "提示", {
          ...
          this.httpPostWithLoading(COREDEPOSITPURCHASE_DELETE_URL, { id: row.id }).then(({ success }) => {
            if (success) {
              this.$message({
                message: "菜单删除成功",
                type: "success"
