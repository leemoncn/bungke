...
<template>
  <el-container style="height: 100%" class="app-container">
    <el-main>
      <div class="filter-container">
        <el-form :inline="true" :model="searchForm" ref="searchForm">
            ...
            <el-form-item label="内容" prop="msg">
              <el-input v-model="searchForm.msg" autocomplete="new-password"/>
            </el-form-item>
      ...
      <div class="filter-container">
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-plus" v-permission="'core_question_add'"
                   @click="onAddButtonPressed">添加
        </el-button>
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-edit" v-permission="'core_question_edit'"
                   @click="onEditButtonPressed">修改
        </el-button>
        <!--<el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-delete" v-permission="'core_question_delete'"-->
      ...
      <el-table-column prop="title" label="标题" sortable="custom" align="center"/>
      <el-table-column prop="msg" label="内容" sortable="custom" align="center"/>
        ...
        <el-table-column label="操作" fixed="right" width="210" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-permission="'core_question_edit'">编辑
            </el-button>
            <!--<el-button-->
              <!--size="mini"-->
              <!--type="danger"-->
              <!--@click.stop="onItemDeleteButtonPressed(scope.row)"-->
              <!--v-permission="'core_question_delete'">删除-->
            <!--</el-button>-->
          </template>
        </el-table-column>
      </el-table>
...
<script>
  import { COREQUESTION_DELETE_URL, COREQUESTION_LIST_URL } from "@/config/host"
  ...
  export default {
    ...
    methods: {
      ...
      proceedListRequest: function (isSearch) {
        this.httpPostWithLoading(COREQUESTION_LIST_URL, this.createListRequestData(isSearch ? this.searchForm : null)).then(({ data, success }) => {
          if (success) {
      ...
      onItemDeleteButtonPressed: function (row) {
        this.$confirm(`删除节点<${row.id}>, 是否继续?`, "提示", {
          ...
          this.httpPostWithLoading(COREQUESTION_DELETE_URL, { id: row.id }).then(({ success }) => {
            if (success) {
              this.$message({
                message: "菜单删除成功",
                type: "success"
