  ...
  <el-container style="height: 100%" class="app-container">
    <el-main>
      <div class="filter-container">
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-plus" v-permission="'core_agency_add'"
                   @click="onAddButtonPressed">添加
        </el-button>
        <el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-edit" v-permission="'core_agency_edit'"
                   @click="onEditButtonPressed">修改
        </el-button>
        <!--<el-button class="filter-item" style="margin-left: 10px;" icon="el-icon-delete" v-permission="'core_agency_delete'"-->
      ...
      <el-table :data="tableData" style="width: 100%" tooltip-effect="dark" border fit @sort-change="onTableSortChange"
        ...
        <el-table-column prop="rechargeByApp" label="充值任务币时平台给上级的返现提成(%)" sortable="custom"/>
        <el-table-column prop="withdrawByUser" label="提现时用户从自己的钱拿出来给上级(%)" sortable="custom"/>
        ...
        <el-table-column label="操作" fixed="right" width="210">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-permission="'core_agency_edit'">编辑
            </el-button>
            <!--<el-button-->
            <!--size="mini"-->
            <!--type="danger"-->
            <!--@click.stop="onItemDeleteButtonPressed(scope.row)"-->
            <!--v-permission="'core_agency_delete'">删除-->
            <!--</el-button>-->
          </template>
        </el-table-column>
      </el-table>
...
<script>
  import { COREAGENCY_DELETE_URL, COREAGENCY_LIST_URL } from "@/config/host"
  ...
  export default {
    ...
    methods: {
      ...
      proceedListRequest: function (isSearch) {
        this.httpPostWithLoading(COREAGENCY_LIST_URL, this.createListRequestData(isSearch ? this.searchForm : null)).then(({ data, success }) => {
          if (success) {
      ...
      onItemDeleteButtonPressed: function (row) {
        this.$confirm(`删除节点<${row.id}>, 是否继续?`, "提示", {
          ...
          this.httpPostWithLoading(COREAGENCY_DELETE_URL, { id: row.id }).then(({ success }) => {
            if (success) {
              this.$message({
                message: "菜单删除成功",
                type: "success"
